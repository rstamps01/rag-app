...
import { Scissors, FileText, Clock, CheckCircle } from 'lucide-react';

const TextProcessingNode = ({ data, isConnectable }) => {
  const { 
    status = 'idle', 
    chunksCreated = 0, 
    processingTime = 0, 
      ...
      <div className="p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg p-3 text-center" style={{
            ...
            <div className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{chunksCreated}</div>
          </div>
          
          <div className="rounded-lg p-3 text-center" style={{
            background: 'var(--bg-secondary)'
          }}>
            <div className="text-xs font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>Chunk Size</div>
            <div className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{chunkSize}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg p-3 text-center" style={{
            ...
            <div className="text-xs font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>Text (chars)*</div>
            <div className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{textExtracted.toLocaleString()}</div>
          </div>
        ...
        <div className="rounded-lg p-3 text-center" style={{
          ...
          <div className="text-xs font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>Language</div>
          <div className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{language.toUpperCase()}</div>
        </div>
...
export default TextProcessingNode;
