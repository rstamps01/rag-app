...
class LLMService:
    """
    LLM Service with singleton pattern, PyTorch SDPA optimization, and automatic model loading
    """
    ...
    def __init__(self):
        ...
        # FIXED: Automatically load Mistral-7B model on initialization
        try:
            self._auto_load_model()
        except Exception as e:
            logger.error(f"Failed to auto-load model during initialization: {e}")
    
    def _auto_load_model(self):
        """Automatically load the default Mistral-7B model"""
        logger.info("Auto-loading Mistral-7B model...")
        success = self.load_model("mistral-7b")
        if success:
            logger.info("Mistral-7B model auto-loaded successfully")
        else:
            logger.error("Failed to auto-load Mistral-7B model")
    ...
    def _check_memory_availability(self, required_memory_gb: float = 7.0) -> bool:
        ...
        try:
            ...
            available_gb = memory_info[0] / (1024**3)
    ...
    def _is_blackwell_architecture(self) -> bool:
        """Check if running on Blackwell architecture (RTX 5090)"""
        if not torch.cuda.is_available():
            return False
        
        try:
            device_props = torch.cuda.get_device_properties(0)
            # Blackwell has compute capability 12.x
            return device_props.major >= 12
        except Exception:
            return False
    ...
    def _load_model(self, model_name: str):
        ...
        try:
            ...
            logger.info(f"Loading model on device: {model_kwargs['device_map']}")
            ...
            # Apply RTX 5090 Blackwell optimizations
            if torch.cuda.is_available():
                if self._is_blackwell_architecture():
                    logger.info("Applying RTX 5090 Blackwell optimizations")
                    torch.set_float32_matmul_precision('high')  # Enable TensorFloat-32
                    
                    # Enable all SDPA backends for optimal performance
    ...
    def generate_response(self, prompt: str, context: str = None, max_new_tokens: int = 512, temperature: float = 0.7) -> str:
        """
        Generate response using the loaded model
        FIXED: Added context parameter and automatic model loading check
        """
        # FIXED: Automatically load model if not loaded
        if self.model is None or self.tokenizer is None:
            logger.warning("No model loaded, attempting to auto-load Mistral-7B...")
            success = self.load_model("mistral-7b")
            if not success:
                raise RuntimeError("No model loaded and failed to auto-load. Call load_model() first.")
...
Answer: """
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blackwell_architecture": self._is_blackwell_architecture(),
