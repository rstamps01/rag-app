#!/usr/bin/env python3
"""
Emergency Main.py Fix Script
Fixes syntax errors and restores backend functionality
"""
...
def backup_current_main():
    """Backup the broken main.py file"""
    try:
        main_path = "/home/vastdata/rag-app-07/backend/app/main.py"
        backup_path = f"{main_path}.syntax-error-backup"
        
        if os.path.exists(main_path):
            shutil.copy2(main_path, backup_path)
            log_message(f"✅ Backed up broken main.py to {backup_path}")
            return True
        else:
            log_message("❌ main.py file not found")
            return False
...
def create_working_main():
    """Create a working main.py file with proper syntax"""
...
'''
    ...
    try:
        ...
        log_message("✅ Created working main.py with proper syntax")
        return True
    except Exception as e:
...
def restart_backend():
    ...
    try:
        ...
        if result.returncode == 0:
            log_message("✅ Backend stopped successfully")
        else:
        ...
        log_message("❌ Backend restart timed out")
        return False
    except Exception as e:
    ...
    for url, name in endpoints:
        try:
            ...
            if response.status_code == 200:
                log_message(f"✅ {name}: Working")
            else:
                log_message(f"⚠️ {name}: HTTP {response.status_code}")
        except requests.exceptions.RequestException as e:
            log_message(f"❌ {name}: Failed - {e}")

def main():
    """Main execution function"""
    print("🚨 RAG Application Emergency Main.py Fix")
    print("=" * 50)
    
    # Step 1: Backup broken file
    log_message("📁 Backing up broken main.py...")
    if not backup_current_main():
        print("❌ Failed to backup current main.py")
        return
    
    # Step 2: Create working file
    log_message("🔧 Creating working main.py...")
    if not create_working_main():
        print("❌ Failed to create working main.py")
        return
    ...
    print("✅ Backend should now be running with proper syntax")
    print("✅ All basic endpoints should be working")
